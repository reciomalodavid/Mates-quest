...
const looksLikeTemplateCode = (text) => /[{}]|\.join\(|^\),|,'/.test(text);
const missingDynamicFragments = [...new Set(dynamicHtmlFragments)].filter(text =>
  !keys.has(text) &&
  !templatePatterns.some(pattern => pattern.test(text)) &&
  !looksLikeTemplateCode(text)
);
...
console.log(`Catalan coverage complete: ${new Set(candidates.map(({ text }) => text)).size} visible strings, ${new Set(dynamicHtmlFragments).size} dynamic HTML fragments and generated messages.`);
